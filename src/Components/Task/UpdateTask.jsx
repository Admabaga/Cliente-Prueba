...
function UpdateTask() {
  ...
  useEffect(() => {
    const fetchTask = async () => {
      try {
        const tasksData = await axios.get(`http://localhost:8080/projects/${project}/tasks/${task}`)
        setTask(tasksData.data);
      } catch (error) {
  ...
  setCargando(true)
  try {
    const response = await axios.put(`http://localhost:8080/users/${user}/projects/${project}/tasks/`, {
        taskName: taskName,
  ...
  return (
    ...
    <FloatingLabel controlId="floatingSelect" label="Tarea a actualizar:" className='mb-3'>
          <Form.Select 
            value={task} 
    ...
    <FloatingLabel controlId="floatingSelect" label="Proyecto:" className='mb-3'>
      ...
      <Form.Group  controlId="exampleForm.ControlTextarea1">
        ...
        <br />
      <Button variant="outline-secondary" onClick={handleSubmit}>Actualizar tarea</Button>{' '}
      {cargando ? (
